...
/**
  * 功能：实现CRC-16/XMODEM  x16+x12+x5+1 的计算，并将计算结果返回
  * 参数：pBuf    待计算的数组
  *       Buflen  数组的长度
  * 返回：CRC-16/XMODEM校验的结果
  */
...
/**
  * 功能：实现CRC-16/XMODEM  x16+x12+x5+1 的计算，并将计算结果添加到数组尾部，并返回
  * 参数：pBuf    待计算的数组
  *       Buflen  数组的长度
  * 返回：CRC-16/XMODEM校验的结果
  */
...
BOOL CRC16_Check( UINT8 * pBuf,UINT16 Buflen)
{
	...
	UINT16 CalCRC=0;
	UINT16 FramCRC=0;
	CalCRC=CRC16_Cal(pBuf+2,Buflen-5);

	FramCRC=((pBuf[Buflen-3]<<8)+pBuf[Buflen-2]);
	if (CalCRC==FramCRC)
	{
		return TRUE;
	}
