...
//  
CString IllusionExcelFile::GetCellString(CString str_path,long irow, long icolumn)
{  
	...
	lpDisp = books.Open(str_path, covOptional, covOptional,
	...
	range.AttachDispatch(range.get_Item(COleVariant((long)irow), COleVariant((long)icolumn)).pdispVal);   //第一变量是行，第二个变量是列，即第二行第一列
	vResult = range.get_Value2();
	CString str;
...
void IllusionExcelFile::SetCellString_Cells(CString tableName, long irow, long icolumn,CString new_string)  
{  
    COleVariant new_value(new_string);  
    CRange start_range = excel_work_sheet_.get_Range(COleVariant(tableName),covOptional);   
    CRange write_range = start_range.get_Offset(COleVariant((long)irow -1),COleVariant((long)icolumn -1) );  
    write_range.put_Value2(new_value);  
    start_range.ReleaseDispatch();  
    write_range.ReleaseDispatch();  
